...
public class Board {
    ...
    public Square insertOnBoard(String chooseWord, String startingCoordinate, String axis, Frame f) throws RankOutOfBoundsException{
		   ...
		   */
	        ...
	        int i = 0;
	        boolean flag = false;
	        char[] arrayOfCharacters = chooseWord.toCharArray(); // 1
	        ...
	        for (char values : arrayOfCharacters ) { // while elements in the array 

  	        	if(f.isTileInFrame(arrayOfCharacters[i])) {
  	        		
  	        		if(axis.equalsIgnoreCase("horizontal")) {
	    	        		//insert horizontally at Coordinate i,j
	    	        		
  	        			// only remove if not already on board
	    	        		removeFromFrame(arrayOfCharacters[i], f);
  	        		}
  	        		
  	        		else if(axis.equalsIgnoreCase("vertical")) {
  	        			//insert vertically at Coordinate i,j
  	        		...
  	        		//check if the tile is already on board and if is increment i,j 
...
public void removeFromFrame (char c, Frame f) {  // deletes a tile from frame after being placed on board
	
	Tile e = null;
	char cUpper = Character.toUpperCase(c);
	
  	if(f.isTileInFrame(c)) {
  		for(i=0; i<7;i++) {
   ...
   /* public Square insertOnBoard(char chooseLetter, String coordinate, Frame f) throws RankOutOfBoundsException{
    ...
    public boolean checkIfWordConflicts( int i, int j){
    ...
    public boolean checkWordLegality(String chooseword, String coordinate , String direction){
    	boolean flag = false; 
    		
    	if (direction.equalsIgnoreCase("horizontal")){
    		for (int i = 0 ; i< chooseword.length() ; i++){
    			if(i == 0 ){
    				if(boardArray[rowCoordinate][columnCoordinate-1].tileInSquareValue == ' ' || boardArray[rowCoordinate+1][columnCoordinate].tileInSquareValue == ' ' && boardArray[rowCoordinate-1][columnCoordinate].tileInSquareValue == ' ') {
    						flag = false ; 
    						continue ; 
    				}
    			}
